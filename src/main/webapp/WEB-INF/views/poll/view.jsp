                        ...
                        <c:choose>
                            <c:when test="${loop.index < poll.options.size()}">
                                ...
                                <div class="d-flex align-items-center mb-2">
                                    <div class="me-2" style="width: 14px; height: 14px; background-color: ${loop.index % 4 == 0 ? 'rgba(13, 110, 253, 0.8)' : loop.index % 4 == 1 ? 'rgba(25, 135, 84, 0.8)' : loop.index % 4 == 2 ? 'rgba(255, 193, 7, 0.8)' : 'rgba(13, 202, 240, 0.8)'}; border-radius: 50%;"></div>
                                    <div class="text-truncate me-auto">
                                        ${option.text}
                                        <c:if test="${poll.userVotedOptionId == option.id}">
                                            <i class="fas fa-check-circle text-success ms-2"></i>
                                            <span class="badge bg-success ms-1"><span data-i18n="yourChoice">您的選擇</span></span>
                                        </c:if>
                                    </div>
                                    <div class="ms-2">${option.voteCount} <span data-i18n="votes">票</span> (${empty poll.totalVotes || poll.totalVotes == 0 ? 0 : Math.round((option.voteCount / poll.totalVotes) * 100)}%)</div>
                                </div>
                            </c:when>
                            <c:otherwise>
                                <!-- Display placeholder for missing options -->
                                <div class="d-flex align-items-center mb-2 text-muted">
                                    <div class="me-2" style="width: 14px; height: 14px; background-color: #e9ecef; border-radius: 50%;"></div>
                                    <div class="text-truncate me-auto">Option ${loop.index + 1} (Not available)</div>
                                    <div class="ms-2">0 <span data-i18n="votes">票</span> (0%)</div>
                                </div>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
