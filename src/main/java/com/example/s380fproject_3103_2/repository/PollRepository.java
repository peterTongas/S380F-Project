...
public interface PollRepository extends JpaRepository<Poll, Long> {
    ...
    @Transactional
    @Query(value = 
        "DELETE FROM USER_VOTES WHERE user_username = :username AND poll_option_id IN " +
        "(SELECT id FROM POLL_OPTIONS WHERE poll_id = :pollId); " +
        "UPDATE POLL_OPTIONS SET vote_count = vote_count - 1 " +
        "WHERE id = :optionId AND vote_count > 0", 
        nativeQuery = true)
    int deleteUserVoteAndDecrementCount(@Param("username") String username, 
                                      @Param("pollId") Long pollId,
